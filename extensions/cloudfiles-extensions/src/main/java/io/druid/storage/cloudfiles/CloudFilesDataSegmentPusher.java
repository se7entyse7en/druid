...
public class CloudFilesDataSegmentPusher implements DataSegmentPusher
{
	...
	@Inject
	public CloudFilesDataSegmentPusher(final CloudFilesApi cloudFilesApi,
		...
		try
		{
			return CloudFilesUtils.retryCloudFilesOperation(new Callable<DataSegment>()
			{
				...
				public DataSegment call() throws Exception
				{
					...
					CloudFilesObject segmentData = new CloudFilesObject(segmentPath, zipOutFile, objectApi.getRegion(),
					...
					CloudFilesObject descriptorData = new CloudFilesObject(segmentPath, descriptorFile,
			        objectApi.getRegion(), objectApi.getContainer());
					log.info("Pushing %s.", descriptorData.getPath());
					objectApi.put(descriptorData);

					final DataSegment outSegment = inSegment.withSize(segmentData.getFile().length())
			        .withLoadSpec(ImmutableMap.<String, Object> of("type", CloudFilesStorageDruidModule.SCHEME,
			            "region", segmentData.getRegion(), "container", segmentData.getContainer(), "path",
			            segmentData.getPath()))
